...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/28 15:05:35 by alngo             #+#    #+#             */
/*   Updated: 2017/07/26 21:38:32 by alngo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static char		*format_unsigned(t_arg *arg, char *buf, size_t size)
{
	char		*tmp;
	size_t		add;
	size_t		i;
...
static int		handle_big_u(t_arg *arg, size_t *len)
{
	char		*buf;
	char		*tmp;
	size_t		size;

	if (!(buf = ft_litob(arg->n, arg, 10, "0123456789abcdef")))
		return (0);
	if (arg->dot && !(arg->n))
		buf[0] = '\0';
	if (!(tmp = format_unsigned(arg, buf, ft_strlen(buf))))
		return (0);
	size = ft_strlen(tmp);
	if (arg->prec)
		arg->flags &= ~ZE;
